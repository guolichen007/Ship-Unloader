...
int main(int argc,char** argv){try{
  ...
  const ship::v15::StructuralRecognizer recognizer;ship::v15::StructuralModelCandidate result;
  if(mode=="ship-frame"){ship::v15::ShipFrameCloud cloud;cloud.points=std::move(points);result=recognizer.recognize(cloud);}else result=recognizer.recognize_offline(points);
  ...
  timing<<"{\"input_point_count\":"<<point_count<<",\"input_ms\":"<<ms(started,loaded)<<",\"recognize_ms\":"<<ms(loaded,recognized)<<",\"serialize_ms\":"<<ms(recognized,written)<<",\"total_ms\":"<<ms(started,written)<<"}\n";
